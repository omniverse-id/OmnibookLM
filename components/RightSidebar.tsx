...
import { PanelRight, MoreVertical, StickyNote, Pen, CircleFadingPlus, FileUp, Files, Trash2 } from 'lucide-react';
...
const ToolItem: React.FC<ToolItemProps> = ({ tool, isCollapsed, onEditClick }) => {
    const Icon = tool.icon;
    const commonClasses = "cursor-pointer group bg-white rounded-lg hover:bg-[#f8f8f7] transition-colors";
    const isAudioOverview = tool.name === 'Audio Overview';
    
    if (isCollapsed) {
        return (
            <div title={tool.name} className={`${commonClasses} w-12 h-12 flex items-center justify-center`}>
    ...
    return (
        <div title={tool.name} className={`${commonClasses} p-3 flex flex-col justify-between h-[100px] border border-gray-200 relative`}>
             {isAudioOverview && onEditClick && (
                 <button 
                    onClick={onEditClick}
                    aria-label="Customize Audio Overview"
                    className="absolute top-2 right-2 p-1.5 rounded-lg text-gray-500 bg-gray-100 hover:bg-gray-200 transition-all duration-200 opacity-0 group-hover:opacity-100"
                >
...
const StudioHomeView: React.FC<StudioHomeViewProps> = ({ onToggle, artifacts, onAddNote, onArtifactSelect, onDelete, setAudioModalOpen }) => {
    ...
    return (
        <div className="flex flex-col h-full relative">
                ...
                <div className="p-4 grid grid-cols-2 gap-2">
                    {studioTools.map(tool => 
                        <ToolItem 
                            ...
                            tool={tool} 
                            isCollapsed={false} 
                            onEditClick={tool.name === 'Audio Overview' ? () => setAudioModalOpen(true) : undefined}
                        />
                    )}
                </div>
                    ...
                    <>
                     ...
                     <>
                        ...
                        <div className="flex flex-col items-center text-center p-6 py-7">
                            <CircleFadingPlus className="w-8 h-8 text-gray-500 mb-4" />
            ...
            <div className="absolute bottom-4 left-0 right-0 flex justify-center pointer-events-none">
                <button
                    onClick={onAddNote}
                    className="h-10 px-5 inline-flex items-center gap-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-md hover:bg-gray-800 transition-colors pointer-events-auto"
                >
...
const RightSidebar: React.FC<RightSidebarProps> = ({ isOpen, onToggle, artifacts, onAddNote, selectedArtifact, onArtifactSelect, onBack, onDelete }) => {
                ...
                {isOpen ? (
                    ...
                    <div className="hidden md:flex flex-col items-center h-full py-3">
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar w-full flex flex-col items-center space-y-2 pt-6">
                            {studioTools.map(tool => <ToolItem key={tool.id} tool={tool} isCollapsed={true} />)}
                        </div>
                        <div className="flex-shrink-0">
                            <button title="Add note" onClick={onAddNote} className="w-12 h-12 rounded-lg bg-gray-900 text-white flex items-center justify-center hover:bg-gray-800 transition-colors">
